-- Fix: Postgres enum/text comparison in client DAM listing RPC.
--
-- Error observed:
--   operator does not exist: dam_asset_type = text
--
-- Cause:
--   dam_assets.asset_type is an enum (dam_asset_type) while RPC params are text.
--   Comparing enum = text fails without casting.
--
-- Fix:
--   Compare using a.asset_type::text for p_type and v_asset_type_slug matches.
