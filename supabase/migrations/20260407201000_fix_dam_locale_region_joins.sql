-- Fix: client DAM listing RPC references non-existent locale/region id columns.
--
-- Error observed:
--   column dl.id does not exist
--
-- Cause:
--   This code joined `dam_locales dl ON dl.id = alm.locale_id` and `dam_regions dr ON dr.id = arm.region_id`,
--   but in this schema locales/regions are keyed by `code` and the mapping tables store `locale_code`/`region_code`.
--
-- Fix:
--   Apply locale/region UI filters directly against `dam_asset_locale_map.locale_code` and
--   `dam_asset_region_map.region_code` (no joins).
--
-- Also keeps the earlier enum/text cast fix for `asset_type`.
