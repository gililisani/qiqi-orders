-- Entitlement-filtered client DAM listing (server-side).
-- Mirrors delivery-time entitlement rules in platform/auth/damAssetAccess.ts:
-- - client must be enabled and have company_id
-- - region restriction applies only if asset has region rows; must intersect company geography
-- - locale restriction applies only if locale codes contain a country subtag (e.g. en-US → US)
-- - audience restriction applies only if company has at least one company_dam_audiences row
-- - archived assets excluded
