...
export const SLIDocument = ({ data }) => {
  ...
  const getWeight = (p) => {
    const direct = Number(p.weight);
    if (!Number.isNaN(direct) && direct > 0) return direct;
    const total = Number(p.total_weight);
    if (!Number.isNaN(total) && total > 0) return total;
    const caseQty = Number(p.case_qty) || 0;
    const caseWeight = Number(p.case_weight) || 0;
    return caseQty * caseWeight;
  };

  rawProducts.forEach((product) => {
    const qty = Number(product.quantity ?? product.total_quantity) || 0;
    const weight = getWeight(product);
    const value = Number(product.value ?? product.total_value) || 0;
    ...
    <Document>
      <Page size="LETTER" style={styles.page}>
        <View style={[{ width: LETTER.live.width }, makeBorder(0.5, 0.5, 0.5, 0.5)]}>
                  ...
                  return (
                    <>
                      <Row>
                        ...
                        <Cell width={col55} bg="#def0fd" justify="flex-start" pad={4} rightBorder={false}>
                      ...
                      <Row top={0.5}>
                        ...
                        <Cell width={col55} borders={makeBorder(0, 0, 0, 0)} rightBorder={false}></Cell>
                      </Row>
                      <Row top={0.5}>
                        <Cell width={col25} bg="#def0fd">
                        ...
                        <Cell width={col30} rightBorder={false}><CB checked={cs.return_to_shipper} label="Return to Shipper" /></Cell>
                      </Row>
                      <Row top={0.5}>
                        <Cell width={col25} bg="#def0fd">
                        ...
                        <Cell width={col55} pad={0} rightBorder={false}>
                          <Row>
                            <Cell width={col55} borders={makeBorder(0, 0, 0, 0)} rightBorder={false}>
          ...
          <Row top={0.5}>
            <Cell width={LETTER.live.width} borders={makeBorder(0, 0, 0, 0)} justify="flex-start" rightBorder={false}>
                    ...
                    <>
                      ...
                      {allProducts.map((product, index) => (
                        <Row key={`${product.hs_code}-${index}`} top={0.5}>
                          ...
                          <Cell width={widths[2]} align="center">{(product.total_quantity || 0).toLocaleString('en-US')}</Cell>
                          <Cell width={widths[3]} align="center">Each</Cell>
                          <Cell width={widths[4]} align="center">{(product.total_weight || 0).toFixed(2)} kg</Cell>
                          <Cell width={widths[5]} align="center">EAR99</Cell>
                          <Cell width={widths[6]} align="center"></Cell>
                          <Cell width={widths[7]}>NLR</Cell>
                          <Cell width={widths[8]} align="center">{formatCurrency(product.total_value || 0)}</Cell>
