...
const styles = StyleSheet.create({
  ...
  labelCell: {
    ...
    fontSize: 6.5,
    fontFamily: 'Helvetica-Bold',
    backgroundColor: '#def0fd', // Light blue background
    borderRightWidth: 0.5,
    borderRightColor: '#000',
    borderRightStyle: 'solid',
  },
...
export const SLIDocument = ({ data }) => {
  ...
  console.log('SLIDocument rendering with products:', products);
  console.log('Checkbox states:', cs);
        ...
        <View style={styles.masterTable}>
          ...
          <View style={styles.fullRow}>
            <View style={{ flexDirection: 'row', height: 48 }}>
              ...
              {/* Left side: Boxes 21-24 */}
              <View style={{ width: '45%', borderRightWidth: 0.5, borderRightColor: '#000', borderRightStyle: 'solid' }}>
                {/* Row 1 */}
                <View style={[styles.nestedRow, { height: 12 }]}>
                  <View style={[styles.labelCell, { width: '55.5%' }]}><Text>21. Shipper Requests Insurance:</Text></View>
                  <View style={[styles.cellLast, { width: '44.5%', flexDirection: 'row' }]}>
                ...
                {/* Row 2 */}
                <View style={[styles.nestedRow, { height: 12 }]}>
                  <View style={[styles.labelCell, { width: '55.5%' }]}><Text>22. Declared Value for Cartage:</Text></View>
                  <View style={[styles.cellLast, { width: '44.5%' }]}><Text></Text></View>
                </View>
                
                {/* Row 3 */}
                <View style={[styles.nestedRow, { height: 12 }]}>
                  <View style={[styles.labelCell, { width: '55.5%' }]}><Text>23. Shipper Must Check:</Text></View>
                  <View style={[styles.cellLast, { width: '44.5%', flexDirection: 'row' }]}>
                ...
                {/* Row 4 */}
                <View style={[styles.nestedRowLast, { height: 12 }]}>
                  <View style={[styles.labelCell, { width: '55.5%' }]}><Text>24. C.O.D Amount:</Text></View>
                  <View style={[styles.cellLast, { width: '44.5%' }]}><Text>$</Text></View>
                </View>
              </View>

              {/* Right side: Box 25 (spans rows 1-2) and deliver to options (rows 3-4) */}
              <View style={{ width: '55%' }}>
                {/* Box 25 label (spans 2 rows height) */}
                <View style={[styles.nestedRow, { height: 24, borderBottomWidth: 0.5, borderBottomColor: '#000', borderBottomStyle: 'solid' }]}>
                  <View style={[styles.labelCellLast, { width: '100%', display: 'flex', justifyContent: 'center', alignItems: 'center' }]}>
                    <Text>25. SHIPPER'S INSTRUCTION in case of inability to deliver consignment as consigned.</Text>
                  </View>
                </View>
                
                {/* Row 3: Abandoned / Return options */}
                <View style={[styles.nestedRow, { height: 12 }]}>
                  <View style={[styles.cell, { width: '50%' }]}><CB checked={cs.abandoned} label="Abandoned" /></View>
                  <View style={[styles.cellLast, { width: '50%' }]}><CB checked={cs.return_to_shipper} label="Return to Shipper" /></View>
                </View>
                
                {/* Row 4: Deliver To */}
                <View style={[styles.nestedRowLast, { height: 12 }]}>
                  <View style={[styles.cellLast, { width: '100%' }]}><CB checked={cs.deliver_to} label="Deliver To:" /></View>
                </View>
              </View>
          ...
          <View style={styles.fullRow}>
            <View>
              ...
              <View style={[styles.nestedRow, { height: productRowHeight }]}>
                <View style={[styles.labelCell, { width: '8%' }]}><Text>27. D/F</Text></View>
                <View style={[styles.labelCell, { width: '18%' }]}><Text>28. Schedule B / HTS Commercial Commodity Desc</Text></View>
                <View style={[styles.labelCell, { width: '10%' }]}><Text>28. SchB/HTS QTY / UOM (if applicable)</Text></View>
                <View style={[styles.labelCell, { width: '12%' }]}><Text>30. DDTC Quantity and DDTC Unit of Measure</Text></View>
                ...
                <View style={[styles.labelCell, { width: '8%' }]}><Text>33. S M E (Y/N)</Text></View>
                <View style={[styles.labelCell, { width: '12%' }]}><Text>34. NLR, Export License No, License Exception Symbol, DDTC Exemption No, DDTC ACM No.,</Text></View>
                <View style={[styles.labelCell, { width: '12%' }]}><Text>35. Value at the Port of Export (US Dollars)</Text></View>
                <View style={[styles.labelCellLast, { width: '10%' }]}><Text>36. License Value by item (if applicable) (US Dollars)</Text></View>
              </View>

              {/* Product Data Rows */}
              {products.map((product, i) => (
                <View key={i} style={[styles.nestedRow, { height: productRowHeight }]}>
                  ...
                  <View style={[styles.cell, { width: '12%' }]}><Text></Text></View>
                  <View style={[styles.cell, { width: '12%' }]}><Text>${(product.value || 0).toFixed(2)}</Text></View>
                  <View style={[styles.cellLast, { width: '10%' }]}><Text></Text></View>
                </View>
              ))}

              {/* Empty rows to ensure minimum 6 product rows */}
              {Array.from({ length: Math.max(0, 6 - products.length) }).map((_, i) => (
                <View key={`empty-${i}`} style={[styles.nestedRow, { height: productRowHeight }]}>
                  <View style={[styles.cell, { width: '8%' }]}><Text></Text></View>
                  <View style={[styles.cell, { width: '18%' }]}><Text></Text></View>
          ...
          <View style={styles.fullRow}>
            <View>
              <View style={[styles.nestedRow, { height: 18 }]}>
                <View style={[styles.cell, { width: '4%', flexDirection: 'row' }]}>
                  <Text style={{ fontSize: 7 }}>39 </Text>
                  <View style={cs.cb_39 ? styles.checkboxChecked : styles.checkbox}>
                    {cs.cb_39 && <View style={styles.checkboxFill} />}
                  </View>
                </View>
                <View style={[styles.cellLast, { width: '96%' }]}>
                  <Text style={{ fontSize: 6.5 }}>Check here if there are any remaining non-licensable Schedule B / HTS Numbers that are valued $2500.00 or less and that do not otherwise require AES filing.</Text>
                </View>
              </View>
              <View style={[styles.nestedRowLast, { height: 30 }]}>
                <View style={[styles.cell, { width: '4%', flexDirection: 'row' }]}>
                  <Text style={{ fontSize: 7 }}>40 </Text>
                  <View style={cs.cb_40 ? styles.checkboxChecked : styles.checkbox}>
                    {cs.cb_40 && <View style={styles.checkboxFill} />}
                  </View>
                </View>
                <View style={[styles.cellLast, { width: '96%' }]}>
                  <Text style={{ fontSize: 6.5 }}>Check here if the USPPI authorizes the above named forwarder to Act as authorized agent for export control, U.S. Census Bureau (Census Bureau) reporting, and U.S. Customs and Border Protection (CBP) purposes. Also, to prepare and transmit any Electronic Export Information (EEI) or other documents or records required to be filed by the Census Bureau, CBP, the Bureau of Industry and Security, or any other U.S. Government agency, and perform any other act that may be required by law or regulation in connection with the exportation or transportation of any goods shipped or consigned by or to the USPPI, and to receive or ship any goods on behalf of the USPPI.</Text>
                </View>
              </View>
            </View>
          </View>
