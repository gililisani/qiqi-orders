...
// Fixed column widths for products grid (LETTER.live.width based)
const GRID_SEP = 0.5; // separator border between cells
const NUM_PROD_COLS = 10;
const INTERNAL_PROD_SEPS = GRID_SEP * (NUM_PROD_COLS - 1);
const COL_W = (() => {
  // Original percentages from HTML: 8,18,10,12,10,10,8,12,12,10 (sum 110)
  const weights = [8, 18, 10, 12, 10, 10, 8, 12, 12, 10];
  const sum = weights.reduce((a, b) => a + b, 0); // 110
  // Subtract internal separators from available width so content + borders == live width
  const available = LETTER.live.width - INTERNAL_PROD_SEPS;
  const scale = available / sum;
  const [w27, w28d, w28q, w30, w31, w32, w33, w34, w35, w36] = weights.map(w => w * scale);
  return {
    c27: w27,
    c28desc: w28d,
    c28qty: w28q,
...
function stripBorderProps(s) {
  ...
  const out = {};
  for (const key in s) {
    if (!banned.has(key)) out[key] = s[key];
  }
  return out;
}
...
function Tr({ children, top = 0, bottom = 0.5, style }) {
  ...
  const userStyle = Array.isArray(style)
    ? style.map(stripBorderProps)
    : stripBorderProps(style);
  ...
  return (
    <View style={[t.row, userStyle, base]}>
      {children}
    </View>
  );
}
...
function Td({
  ...
  // Normalize width: allow numbers (pts) or percentage strings like "30%"
  let normalizedWidth;
  if (typeof width === 'string' && width.trim().endsWith('%')) {
    const pct = parseFloat(width);
    if (!Number.isNaN(pct)) normalizedWidth = (LETTER.live.width * pct) / 100;
  ...
  if (normalizedWidth !== undefined) base.width = normalizedWidth;
  if (bg !== undefined) base.backgroundColor = bg;

  return (
    <View style={[t.cell, userStyle, base]}>
    ...
    <Document>
      <Page size="LETTER" style={styles.page}>
        ...
        <Table border={0.5} style={{ width: LETTER.live.width }}>
            ...
            <Td right={0} pad={0}>
              <Table border={0}>
                ...
                <Tr bottom={0}>
                  ...
                  <Td width="30%" right={0.5}>United States</Td>
                  <Td width="20%" bg="#def0fd" right={0.5}><Text style={styles.textBold}>6. Date of Export:</Text></Td>
                  <Td width="20%" right={0}>{data.date_of_export || new Date().toLocaleDateString('en-US')}</Td>
                </Tr>
              </Table>
            </Td>
          </Tr>
          ...
          {/* Row 6: Insurance/Payment Section (flat grid) */}
          <Tr bottom={0.5}>
            <Td right={0} pad={0}>
              <View style={styles.gridRow}>
                <View style={[styles.gridCell, { width: LETTER.live.width * 0.25 }, styles.gridLabel]}>
                  <Text style={styles.textBold}>21. Shipper Requests Insurance:</Text>
                </View>
                <View style={[styles.gridCell, { width: LETTER.live.width * 0.20 }]}>
                  <View style={styles.checkboxContainer}>
                    <CB checked={cs.insurance_yes} label="Yes" />
                    <CB checked={cs.insurance_no} label="No" />
                  </View>
                </View>
                <View style={[styles.gridCellLast, { width: LETTER.live.width * 0.55, minHeight: 30 }, styles.gridLabel]}>
              ...
              <View style={styles.gridRow}>
                ...
                <View style={[styles.gridCellLast, { width: LETTER.live.width * 0.20 }]}></View>
                <View style={[styles.gridCellLast, { width: LETTER.live.width * 0.55 }]}></View>
              </View>
              <View style={styles.gridRow}>
                <View style={[styles.gridCell, { width: LETTER.live.width * 0.25 }, styles.gridLabel]}>
                ...
                <View style={[styles.gridCell, { width: LETTER.live.width * 0.275 }]}>
                  <CB checked={cs.abandoned} label="Abandoned" />
                </View>
                <View style={[styles.gridCellLast, { width: LETTER.live.width * 0.275 }]}>
              ...
              <View style={styles.gridRow}>
                ...
                <View style={[styles.gridCellLast, { width: LETTER.live.width * 0.55 }]}>
          ...
          {/* Row 10: Products Table (flat grid) */}
          <Tr bottom={0.5}>
            <Td right={0} pad={0}>
              {/* Header */}
              <View style={styles.gridRowHeader}>
                <View style={[styles.gridCell, { width: COL_W.c27 }, styles.gridLabel]}><Text style={styles.textBold}>27. D/F</Text></View>
                <View style={[styles.gridCell, { width: COL_W.c28desc }, styles.gridLabel]}><Text style={styles.textBold}>28. Schedule B / HTS Commercial Commodity Desc</Text></View>
                <View style={[styles.gridCell, { width: COL_W.c28qty }, styles.gridLabel]}><Text style={styles.textBold}>28. SchB/HTS QTY / UOM (if applicable)</Text></View>
                <View style={[styles.gridCell, { width: COL_W.c30 }, styles.gridLabel]}><Text style={styles.textBold}>30. DDTC Quantity and DDTC Unit of Measure</Text></View>
                <View style={[styles.gridCell, { width: COL_W.c31 }, styles.gridLabel]}><Text style={styles.textBold}>31. Shipping Weight (in Kilos)</Text></View>
                <View style={[styles.gridCell, { width: COL_W.c32 }, styles.gridLabel]}><Text style={styles.textBold}>32. ECCN EAR99 or USML Category #</Text></View>
                <View style={[styles.gridCell, { width: COL_W.c33 }, styles.gridLabel]}><Text style={styles.textBold}>33. S M E (Y/N)</Text></View>
                <View style={[styles.gridCell, { width: COL_W.c34 }, styles.gridLabel]}><Text style={styles.textBold}>34. NLR, Export License No, License Exception Symbol, DDTC Exemption No, DDTC ACM No.,</Text></View>
                <View style={[styles.gridCell, { width: COL_W.c35 }, styles.gridLabel]}><Text style={styles.textBold}>35. Value at the Port of Export (US Dollars)</Text></View>
                <View style={[styles.gridCellLast, { width: COL_W.c36 }, styles.gridLabel]}><Text style={styles.textBold}>36. License Value by item (if applicable) (US Dollars)</Text></View>
              </View>
              {/* Rows */}
              {allProducts.map((product, index) => (
                <View key={index} style={styles.gridRow}>
                  <View style={[styles.gridCell, { width: COL_W.c27 }]}><Text style={styles.text}>{getDorF(product.made_in)}</Text></View>
                  <View style={[styles.gridCell, { width: COL_W.c28desc }]}><Text style={styles.text}>{product.hs_code}</Text></View>
                  <View style={[styles.gridCell, { width: COL_W.c28qty }]}><Text style={styles.text}>{product.total_quantity.toLocaleString('en-US')}</Text></View>
                  <View style={[styles.gridCell, { width: COL_W.c30 }]}><Text style={styles.text}>Each</Text></View>
                  <View style={[styles.gridCell, { width: COL_W.c31 }]}><Text style={styles.text}>{product.total_weight.toFixed(2)} kg</Text></View>
                  <View style={[styles.gridCell, { width: COL_W.c32 }]}><Text style={styles.text}>EAR99</Text></View>
                  <View style={[styles.gridCell, { width: COL_W.c33 }]}></View>
                  <View style={[styles.gridCell, { width: COL_W.c34 }]}><Text style={styles.text}>NLR</Text></View>
                  <View style={[styles.gridCell, { width: COL_W.c35 }]}><Text style={styles.text}>${product.total_value.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</Text></View>
                  <View style={[styles.gridCellLast, { width: COL_W.c36 }]}></View>
                </View>
              ))}
            </Td>
          </Tr>
