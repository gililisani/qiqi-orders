...
function Tr({ children, top = 0, bottom = 0.5, style }) {
  ...
  // Only add border widths if they're numbers (not undefined/null)
  if (typeof top === 'number') rowStyle.borderTopWidth = top;
  if (typeof bottom === 'number') rowStyle.borderBottomWidth = bottom;
  
  // Filter out undefined values from style prop
  const cleanStyle = style ? Object.fromEntries(
    Object.entries(style).filter(([_, v]) => v !== undefined)
  ) : {};
  
  return (
    <View style={[t.row, rowStyle, cleanStyle]}>
      {children}
    </View>
  );
}
...
function Td({ children, right = 0.5, left = 0, pad = 2, style, align = "left", valign = "middle", width, bg }) {
  ...
  // Only add border widths if they're numbers (not undefined/null)
  if (typeof right === 'number') cellStyle.borderRightWidth = right;
  if (typeof left === 'number') cellStyle.borderLeftWidth = left;
  ...
  return (
    <View style={[t.cell, cellStyle, cleanStyle]}>
