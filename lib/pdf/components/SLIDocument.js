...
function Table({ children, border = 0.5, style }) {
  return <View style={[t.table, style, { borderWidth: border }]}>{children}</View>;
}

function Tr({ children, top = 0, bottom = 0.5, style }) {
  return (
    <View style={[t.row, style, { borderTopWidth: top, borderBottomWidth: bottom, borderStyle: "solid" }]}>
      {children}
    </View>
  );
}

function Td({ children, right = 0.5, left = 0, pad = 2, style, align = "left", valign = "middle", width, bg }) {
  const cellStyle = {
  ...
  // Only add width if defined
  if (width !== undefined) cellStyle.width = width;
  
  // Only add backgroundColor if defined
  if (bg !== undefined) cellStyle.backgroundColor = bg;
  
  return (
    <View style={[t.cell, cellStyle, style]}>
        ...
        <Table border={0.5} style={{ width: LETTER.live.width }}>
              ...
              <Table border={0}>
                ...
                {allProducts.map((product, index) => (
                  <Tr key={index} bottom={index === allProducts.length - 1 ? 0 : 0.5}>
                    <Td width="8%" right={0.5}>{getDorF(product.made_in)}</Td>
                    <Td width="18%" right={0.5}>{product.hs_code}</Td>
                    <Td width="10%" right={0.5}>{product.total_quantity.toLocaleString('en-US')}</Td>
                    <Td width="12%" right={0.5}>Each</Td>
                    <Td width="10%" right={0.5}>{product.total_weight.toFixed(2)} kg</Td>
                    <Td width="10%" right={0.5}>EAR99</Td>
                    <Td width="8%" right={0.5}></Td>
                    <Td width="12%" right={0.5}>NLR</Td>
                    <Td width="12%" right={0.5}>${product.total_value.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</Td>
          ...
          <Tr bottom={0.5}>
            <Td right={0} pad={0}>
              <Table border={0}>
                <Tr bottom={0.5}>
                  <Td width="8%" right={0.5}>39 <CB checked={cs.checkbox_39} label="" /></Td>
                ...
                <Tr bottom={0}>
                  <Td width="8%" right={0.5}>40 <CB checked={cs.checkbox_40} label="" /></Td>
          ...
          <Tr bottom={0.5}>
            <Td right={0}>
              48. <CB checked={cs.checkbox_48} label="Check here to validate Electronic Signature. Electronic signatures must be typed in all capital letters in Box 38 in order to be valid." />
            </Td>
          </Tr>
