...
function Tr({ children, top = 0, bottom = 0.5, style }) {
  // Coerce to numbers, never undefined
  ...
  return (
    <View
      style={[
        ...
        style || {}, // pass-through, don't filter; RN styles can be arrays/ids
      ]}
    >
      {children}
    </View>
...
function Td({
  ...
  style,
  align = 'left',
  valign = 'top', // use 'top'/'center'/'bottom'
  width,
  bg,
}) {
  ...
  const base = {
    display: 'table-cell',
    verticalAlign: valign, // 'top' | 'center' | 'bottom'
    borderStyle: 'solid',
