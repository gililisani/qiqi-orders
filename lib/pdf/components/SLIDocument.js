  ...
  <Cell width={LETTER.live.width} borders={makeBorder(0, 0, 0, 0)} pad={0}>
    <View>
      {(() => {
        ...
        const col25b = pct(25);
        const col30b = pct(30);
        const wLeft = col25 + col20;   // 45%
        const wRight = pct(55);        // 55%

        // --- BLOCK A: first two logical rows on the left as a nested table,
        //              with a single right cell that naturally grows to the same height (rowspan).
        return (
          <>
            ...
            <Row>
              ...
              <View style={{ width: wLeft }}>
                ...
                <Row>
                  ...
                  {/* remove right border here; seam will be drawn by the right cell */}
                ...
                <Row top={0.5}>
                  ...
                  {/* also no right border to avoid double seam */}
                  <Cell width={col20} rightBorder={false}>
              ...
              {/* RIGHT cell (rowspan=2 visually) */}
              <Cell
                width={wRight}
                justify="flex-start"
                pad={4}
                // draw a left border so it meets the left nested table without doubling
                borders={makeBorder(0, 0.5, 0, 0.5)}
                rightBorder={false}
              >
                <Text style={styles.textBold} bg="#def0fd">
            ...
            {/* A3: row 4 (colspan on last two columns via a nested table) */}
            <Row top={0.5}>
              <Cell width={col25} bg="#def0fd">
              ...
              {/* colspan = col3+col4 -> build a nested table inside this single wide cell */}
              <Cell width={col25b + col30b} pad={0} rightBorder={false}>
                <View>
                  {/* inner nested table, one full-width row */}
                  <Row>
                    <Cell
                      width={col25b + col30b}
                      // keep-all-borders equivalent: draw borders on this inner row,
                      // but avoid doubling outer right border
                      borders={makeBorder(0, 0, 0, 0)}
                      rightBorder={false}
                    >
          ...
          <Row top={0.5}>
            <Cell width={LETTER.live.width} borders={makeBorder(0, 0, 0, 0)} justify="flex-start" rightBorder={false} pad={8} style={{ minHeight: 34 }}>
