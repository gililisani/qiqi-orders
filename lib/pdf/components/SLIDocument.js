...
{/* Row 6: Insurance / Payment & Box 25 (with rowspan & colspan) */}
<Row top={0.5}>
  <Cell width={LETTER.live.width} borders={makeBorder(0, 0, 0, 0)} pad={0}>
    <View>
      {(() => {
        // Left grid (two rows): 25% + 20% = 45%
        const col25 = pct(25);
        const col20 = pct(20);
        const wLeft = col25 + col20;

        // Right merged cell spans the two left rows
        const wRight = pct(55);
        const rowH = 14; // tweak if needed

        // 4-col grid for rows 3–4
        const widths4 = [pct(25), pct(20), pct(25), pct(30)];

        return (
          <>
            {/* Rows 1–2 (left stack) + right merged cell (rowspan=2) */}
            <Row>
              {/* LEFT: two rows stacked */}
              ...
              {/* RIGHT: merged cell spanning two rows */}
              <Cell
                width={wRight}
                justify="flex-start"
                pad={4}
                style={{ minHeight: rowH * 2 + 0.5 }}
                rightBorder={false}
              >
            ...
            {/* Row 4: colspan across last two columns */}
            <Row top={0.5}>
              <Cell width={widths4[0]} bg="#def0fd">
