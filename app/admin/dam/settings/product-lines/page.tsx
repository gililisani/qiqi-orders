...
export default function ProductLinesPage() {
  ...
  const fetchItems = async () => {
    try {
      setLoading(true);
      const res = await fetchWithAuth('/api/admin/product-lines');
      if (!res.ok) {
        const data = await res.json().catch(() => ({}));
        throw new Error(data.error || 'Failed to load product lines');
      }
      const data = await res.json();
      setItems(data.productLines || []);
      setError(null);
    } catch (err: any) {
  ...
  const handleToggleActive = async (item: ProductLine) => {
    try {
      const res = await fetchWithAuth('/api/admin/product-lines', {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
  ...
  const handleDelete = async (item: ProductLine) => {
    ...
    if (!ok) return;
    try {
      const res = await fetchWithAuth(`/api/admin/product-lines?id=${item.id}`, {
        method: 'DELETE',
      });
      if (!res.ok) {
        const data = await res.json();
  ...
  return (
    <div className="px-6 py-8 space-y-4">
      ...
      <PageHeader
        title="Product lines"
        description="Product line options for tagging assets."
        actions={
      ...
      <Card>
        {loading ? (
          ...
          <EmptyState
            ...
            description="Add product lines to start tagging assets."
            action={
              <Button size="sm" onClick={() => setShowCreateModal(true)}>
                <Plus className="h-4 w-4" /> Add product line
              </Button>
