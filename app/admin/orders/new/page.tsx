...
export default function AdminNewOrderPage() {
  ...
  const getClientType = () => {
    if (!selectedCompany?.class?.name) return 'Americas';
    
    const className = selectedCompany.class.name.toLowerCase();
    return className.includes('international') ? 'International' : 'Americas';
  };

  const getProductPrice = (product: Product) => {
    if (!selectedCompany?.class?.name) return product.price_americas;
  ...
  const fetchCompanies = async () => {
    try {
      ...
      const transformedCompanies = companiesData?.map(company => ({
        ...company,
        class: Array.isArray(company.class) ? company.class[0] : company.class,
        support_fund: Array.isArray(company.support_fund)
          ? company.support_fund
          : (company.support_fund ? [company.support_fund] : [])
      })) || [];
  ...
  const getOrderTotals = () => {
    ...
    const rawSf = selectedCompany?.support_fund as any;
      ...
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          <h1 className="text-2xl font-bold text-gray-900">
            {showSupportFundRedemption ? 'Redeem Support Funds' : 'New Order (Admin)'}
          </h1>
          <Link
            href="/admin/orders"
        ...
        <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
          {/* Left: company selection + products */}
          <div className="xl:col-span-2 space-y-6">
            ...
            {/* Tab Navigation */}
            {selectedCompany && totals.supportFundPercent > 0 && (
              <Card>
                    ...
                    {orderItems.length > 0 ? (
                      <>
                        ...
                        <div className="flex flex-col space-y-2">
                          {showSupportFundRedemption ? (
                            <Button
                              onClick={() => handleSubmitWithSupportFund()}
