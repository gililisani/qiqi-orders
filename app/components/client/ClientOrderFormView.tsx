...
/**
 * ClientOrderFormView — partner-facing order create/edit form.
 *
 * Forked from AdminOrderFormView. Differences vs admin:
 *   - No company picker — the client is bound to their own company,
 *     fetched from the clients table on mount.
 *   - Otherwise: same tabbed cart, accordion product browser, support
 *     fund logic, save-as-draft reminder dialog.
 *
 * Reuses the shared useOrderFormController hook with role: 'client'
 * so redirect targets and audit log entries match the existing flow.
 */
...
interface ClientOrderFormViewProps {
  orderId?: string | null;
  backUrl: string;
}
...
// Main
// ----------------------------------------------------------------------------
export default function ClientOrderFormView({ orderId, backUrl }: ClientOrderFormViewProps) {
  ...
  const recalcCartScrollHint = () => {
    ...
    if (!el) return;
    setCartHasMoreBelow(el.scrollHeight - el.scrollTop - el.clientHeight > 4);
  };
  ...
  // Auto-scroll on single add; recheck hint on any change
  useEffect(() => {
  ...
  // ---- Load client's company + products ----
  const fetchClientCompany = async () => {
    ...
    const c = Array.isArray((data as any)?.company)
      ? (data as any).company[0]
      : (data as any)?.company;
    if (!c) throw new Error('No company linked to your account.');
    return c as Company;
  };

  const fetchProductsForCompany = async (_companyData: Company) => {
    const { data, error } = await supabase
      .from('Products')
    ...
    (async () => {
      try {
        if (isEditMode) {
          ...
          const c = await fetchClientCompany();
          setCompany(c);
          setOrder({ id: '', po_number: '', status: 'Open', company_id: c.id });
  ...
  // Class names in NS look like "North America Distributors" or
  // "International Distributors" — match by substring so we don't depend
  // on exact wording. Any class whose name mentions "america" maps to
  // price_americas; everything else (including no class) falls back to
  // price_international.
  ...
  const handleCaseQtyChange = (productId: number, newCaseQty: number) => {
    ...
    setOrderItems((prev) => {
      ...
      if (newCaseQty === 0) return prev.filter((i) => i.product_id !== productId);
      if (existing) {
        return prev.map((i) =>
          ...
          <table className="w-full" style={{ tableLayout: 'fixed' }}>
            <thead>
              <tr className="bg-muted/40 border-y border-border">
                <th className="px-3 py-2 text-left text-[10px] font-semibold uppercase tracking-wider text-muted-foreground" style={{ width: '46%' }}>Product</th>
                <th className="px-2 py-2 text-center text-[10px] font-semibold uppercase tracking-wider text-muted-foreground hidden sm:table-cell" style={{ width: '10%' }}>SKU</th>
                <th className="px-2 py-2 text-center text-[10px] font-semibold uppercase tracking-wider text-muted-foreground hidden xl:table-cell" style={{ width: '7%' }}>Size</th>
                <th className="px-2 py-2 text-center text-[10px] font-semibold uppercase tracking-wider text-muted-foreground hidden xl:table-cell" style={{ width: '7%' }}>Pack</th>
                <th className="px-2 py-2 text-right text-[10px] font-semibold uppercase tracking-wider text-muted-foreground" style={{ width: '12%' }}>Price</th>
                <th className="px-1 py-2 text-center text-[10px] font-semibold uppercase tracking-wider text-muted-foreground" style={{ width: '14%' }}>Qty</th>
                <th className="px-2 py-2 text-center text-[10px] font-semibold uppercase tracking-wider text-muted-foreground hidden sm:table-cell" style={{ width: '8%' }}>Units</th>
                <th className="px-2 py-2 text-right text-[10px] font-semibold uppercase tracking-wider text-muted-foreground" style={{ width: '12%' }}>Total</th>
              </tr>
            </thead>
            <tbody>
